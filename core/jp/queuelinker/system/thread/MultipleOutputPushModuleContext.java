...
@NotThreadSafe
public class MultipleOutputPushModuleContext<I extends Serializable, O extends Serializable>
							extends ThreadContextBase implements InputPushContext<I>, OutputContext<O> {
	private final MultipleOutputPushModule<I, O> instance;
	...
	MultipleOutputPushModuleContext(final PushModulesThreadUnit unit, final MultipleOutputPushModule<I, O> instance, final QueueLinkerService service) {
		super(unit);
	...
	void execute() {
		...
		final O[] results = instance.execute(i, service);
		cpuCycles += UnsafeUtil.rdtsc() - start;
		waitingCount--;
		outputCount += results.length;
		for (O o : results)
			outputRouter.send(threadLocalId, 0, o);
	...
	public String toString() {
		return String.format("MultipleOutputPushModuleContext (%s)", instance.getClass().getCanonicalName());
	}
